//
// Created by AliGriv on 2021-02-10.
//
...
//            }
            if ( c >= '1' && c <= '9' )
            {
                ...
                std::cout << "g_discoveredServers.size() is " << g_discoveredServers.size() << std::endl;
            ...
            if (g_discoveredServers.size() < 1 && (std::chrono::steady_clock::now() - start > std::chrono::milliseconds(2000))) {
                std::cout << "no server found and time exceeded 2 secs" << std::endl;
                return 0;
            }
        }
...
//        c = '1';
//    }
    while(c=getch() && (std::chrono::steady_clock::now() - start < std::chrono::milliseconds(5)))
//    while(c)
    {
//        if(std::chrono::steady_clock::now() - start > std::chrono::milliseconds(100)) {
//            c = 'q';
...
//        }
    ...
    if (g_pClient)
    {
        std::cout << "going to run g_pClient->Disconnect()" << std::endl;
...
//    printf("Timecode : %s\n", szTimecode);
    ...
    std::vector<data_NatNet> temp_data;
    for(i=0; i < data->nRigidBodies; i++)
    {
        data_NatNet temp_data2;
        // params
...
//               data->RigidBodies[i].qw);
        temp_data2.pos.at(0) = data->RigidBodies[i].x;
        temp_data2.pos.at(1) = data->RigidBodies[i].y;
        temp_data2.pos.at(2) = data->RigidBodies[i].z;
        temp_data2.orient.at(0) = data->RigidBodies[i].qx;
        temp_data2.orient.at(1) = data->RigidBodies[i].qy;
        temp_data2.orient.at(2) = data->RigidBodies[i].qz;
        temp_data2.orient.at(3) = data->RigidBodies[i].qw;
        temp_data2.bTrackingValid = bTrackingValid;
        temp_data2.ID = data->RigidBodies[i].ID;
        test_data->pos.at(0) = data->RigidBodies[i].x;
        test_data->pos.at(1) = data->RigidBodies[i].y;
        test_data->pos.at(2) = data->RigidBodies[i].z;
        test_data->orient.at(0) = data->RigidBodies[i].qx;
        test_data->orient.at(1) = data->RigidBodies[i].qy;
        test_data->orient.at(2) = data->RigidBodies[i].qz;
        test_data->orient.at(3) = data->RigidBodies[i].qw;
        test_data->bTrackingValid = bTrackingValid;
        test_data->ID = data->RigidBodies[i].ID;
        temp_data.push_back(temp_data2);
    }
...
//void client_NatNet::test_function() {
//    std::cout << "here in the test function" << std::endl;
//}

void test_class::do_something() {
